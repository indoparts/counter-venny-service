...
export default class AbsensiController {
    public async index({ bouncer, response, request }: HttpContextContract) {
        try {
            await bouncer.authorize("read-absensi")
            if (await bouncer.allows('read-absensi')) {
    ...
    public async store({ bouncer, request, response, auth }: HttpContextContract) {
        try {
            await bouncer.authorize("create-absensi")
            if (await bouncer.allows('create-absensi')) {
    ...
    public async destroy({ bouncer, request, response }: HttpContextContract) {
        try {
            await bouncer.authorize("delete-absensi")
            if (await bouncer.allows('delete-absensi')) {
    ...
    public async laporan({ bouncer, response, request }: HttpContextContract) {
        try {
            await bouncer.authorize("report-absensi")
            if (await bouncer.allows('report-absensi')) {
                const { sortBy, sortDesc, page, limit, between, search } = request.all()
